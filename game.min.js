...
else{ig.music.pause();}}}}});EntityButtonBack=EntityButton.extend({size:{x:186,y:58},animSheet:animSheetList.buttonBack[_LANG_CODE],update:function(){if(ig.input.pressed('click')&&this.inFocus()){if(ig.Sound.enabled)
...
this.counter++;this.parallax.move(40);this.parent();},draw:function(){this.parent();this.parallax.draw();this.BtnStart.draw();this.BtnAudio.draw();var range=((this.counter/3)%60)-30;var delta=(range>0)?1:-1;var tick=range*delta;this.ship.draw(-30,ig.system.height-this.ship.height+tick+10);this.logo.draw((ig.system.width-this.logo.width)/2,10);this.title.draw((ig.system.width-this.title.width+160)/2,100);this.bestScore.draw(10,ig.system.height-10-50);this.font.draw(this.oldHighscore,70,ig.system.height-28,ig.Font.ALIGN.CENTER);}});StoryScreen=ig.Game.extend({story:animSheetList.screenStory[_LANG_CODE],buttonContinue:animSheetList.buttonContinueImg[_LANG_CODE],init:function(){ig.input.bind(ig.KEY.SPACE,'start');ig.input.bind(ig.KEY.ENTER,'start');this.BtnContinue=this.spawnEntity(EntityButtonContinueHowTo,ig.system.width-132-10,ig.system.height-43-10);},update:function(){if(ig.input.pressed('start')){ig.system.setGame(HowToScreen);}
if(!this.BtnContinue)
...
this.BtnContinue.draw();}});ig.Sound.enabled=true;
...
var buttonEvent = function(e) {
	if (e.keyName == "back") {
		if(ig.game.BtnPause && !ig.game.gamePaused){
...
if (document[hidden]){
    ...
    document.querySelector("html").style.backgroundImage = 'url(media/img/deepspace.png)';
	ig.music.play()
    }
}
document.addEventListener(visibilityChange, handleVisibilityChange, false);
