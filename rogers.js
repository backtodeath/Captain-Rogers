...
var animSheetList = {
	...
	animSheetList.UIScore[lang] = new ig.AnimationSheet('media/img/lang/'+lang+'/ui-score.png',150, 20);
	animSheetList.UIShield[lang] = new ig.AnimationSheet('media/img/lang/'+lang+'/ui-shield.png',100, 20);
...
EntityButton = ig.Entity.extend({
	...
	inFocus: function() {
		var element = document.getElementById('game')['style']['-webkit-transform'];
		var scaleText = element.substr(element.indexOf("scale") + 6);
		var scale = scaleText.substr(0, scaleText.length - 1);
		ig.input.mouse.y = ig.input.mouse.y+4;
		return (
			(this.pos.x <= (ig.input.mouse.x * scale + ig.game.screen.x)) &&
			((ig.input.mouse.x * scale + ig.game.screen.x) <= this.pos.x + this.size.x) &&
			(this.pos.y <= (ig.input.mouse.y * scale  + ig.game.screen.y )) &&
			((ig.input.mouse.y * scale  + ig.game.screen.y ) <= this.pos.y  + this.size.y)
		);
	}
});
...
RogersGame = ig.Game.extend({
	...
	gamePausedScreen: new ig.Image(getImageUrl('pause')),
	gameCompletedScreen: new ig.Image(getImageUrl('gamecompleted')),
...
StartScreen = ig.Game.extend({
	...
	init: function() {
		...
		this.BtnAudio = this.spawnEntity(EntityButtonAudio, ig.system.width-47, 10);
...
StoryScreen = ig.Game.extend({
	story: new ig.Image(getImageUrl('story')),
...
HowToScreen = ig.Game.extend({
	howTo: new ig.Image(getImageUrl('howto')),
	buttonContinue: animSheetList.buttonContinueImg[_LANG_CODE],
	init: function() {
		ig.input.bind(ig.KEY.SPACE, 'start');
		ig.input.bind(ig.KEY.ENTER, 'start');
...
ig.main('#canvas', StartScreen, 0, window.innerWidth, window.innerHeight, 1, ig.ImpactSplashLoader);
});
